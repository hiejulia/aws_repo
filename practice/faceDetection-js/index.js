//this is the face detection demo 
...
exports.handler = function(event, context) {
  ...
  var imageUrl = event.imageUrl;//get the input image url 
  request.get(imageUrl, function (err, res, body) {
    if (err) {
      console.log(err);
      context.fail(err);
    }
    //opencv read input image
    cv.readImage(body, function(err, im) {
      ...
      im.detectObject("node_modules/opencv/data/haarcascade_frontalface_alt.xml", {}, function(err, faces) {
        if (err) context.fail(err);//fail if input image has no width
        for (var i = 0; i < faces.length; i++){
          var face = faces[i];
          im.rectangle([face.x, face.y], [face.width, face.height], [255, 255, 255], 2);//draw the faces
        }
        if (faces.length > 0) {
          var dstKey = dstPrefix + getFormattedDate() + '-' + uuid.v4() + '.jpg';
          var contentType = 'image/jpeg';
          //upload the image with the detected to s3 
          s3.putObject({
            Bucket: dstBucket,
            Key: dstKey,
            Body: im.toBuffer(),
